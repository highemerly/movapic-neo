...
- **画像処理**: sharp + skia-canvas（サーバーサイド）。HEIC/HEIF入力は sharp が system libvips（libheif/libde265）経由でデコード（詳細は「HEIC対応」セクション）
- **言語**: TypeScript
...
HEIC/HEIF は sharp が **system libvips（libheif/libde265）** でデコードする（専用変換コードなし）。要点だけ:
...
- ⚠️ **heic-convert を再導入しない**（純JS/WASMで遅く生成が504タイムアウトしたため廃止。commit `aec8325`）。
