...
- 全て日本語で回答。
- 本番環境でも実行するコマンド（データベースのマイグレーション・本番環境でも実行すべきバックフィルスクリプトなど）は、手順確認も兼ねてユーザーが実行するので、検証環境であっても勝手に実行しない。一方、テストで値を確認したり、状況を確認するようなコマンドは勝手にやる。
- Prismaクライアントの再生成が必要ならば勝手にやる。
- 勝手にコミットしない（ユーザーが手でコミットする）。
- リバート時はgitコマンドを使わず、ファイルをひとつひとつ修正する。
...
## ドキュメント索引（詳細はこちら）
必要になった領域を触るときに読むこと。CLAUDE.md には常時必要な要点のみ残す。
