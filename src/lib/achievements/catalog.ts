...
/**
 * セクション（カテゴリ）の表示順。実績を並べる側（/stats の取得率一覧など）はこの順を使う。
 *
 * 固定カタログの `section` に加えて、resolveAchievement が動的キーに返すセクション
 * （皆勤賞・未知キーのフォールバック「その他」）も含める。並べる側が自前の順序表を持つと、
 * ここにセクションを足したときに片方だけ直して無言で末尾送りになるため単一ソースにする。
 */
