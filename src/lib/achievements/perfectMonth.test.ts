...
/** assignMonthMakeups の Map を素の object にして比較しやすくする。 */
function assignedObj(posts: ReadonlyArray<{ id: string; day: number }>) {
  return Object.fromEntries(assignMonthMakeups(posts));
}
...
describe("pickMakeupHole - 逐次貪欲の割当規則（穴埋めの中核）", () => {
  it("ダブル投稿日でなければ穴を埋めない（count<2 → null）", () => {
    ...
    expect(
      pickMakeupHole({ dayCounts: dc([0, 1, 1]), filledHoleDays: [], postDay: 3, postDayHasDonor: false })
    ).toBeNull();
  });
  ...
  it("ダブル投稿日は自分より前の最古の空き日を埋める", () => {
    ...
    expect(
      pickMakeupHole({ dayCounts: dc([0, 0, 2]), filledHoleDays: [], postDay: 3, postDayHasDonor: false })
    ).toBe(1);
  });
  ...
  it("既に埋め済みの日はスキップして次の空き日を返す", () => {
    ...
    expect(
      pickMakeupHole({ dayCounts: dc([0, 0, 2]), filledHoleDays: [1], postDay: 3, postDayHasDonor: false })
    ).toBe(2);
  });

  it("その日に既に donor がいれば埋めない（1日1donor）", () => {
    expect(
      pickMakeupHole({ dayCounts: dc([0, 0, 2]), filledHoleDays: [], postDay: 3, postDayHasDonor: true })
    ).toBeNull();
  });
  ...
  it("将来日は埋められない（穴は postDay より前だけ）", () => {
    ...
    expect(
      pickMakeupHole({ dayCounts: dc([1, 1, 1, 1, 2, 0]), filledHoleDays: [], postDay: 5, postDayHasDonor: false })
    ).toBeNull();
  });

  it("自分より前がすべて投稿済みなら埋める穴が無く null", () => {
    expect(
      pickMakeupHole({ dayCounts: dc([1, 1, 2]), filledHoleDays: [], postDay: 3, postDayHasDonor: false })
    ).toBeNull();
  });
});
