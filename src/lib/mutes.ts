 ...
 * 公開タイムライン／同じサーバー／ランダムから隠し、さらに mutedUser からのお気に入り／
 * リアクション通知と、リアクション者一覧の表示（自分の投稿詳細・プロフィールカードの
 * アバター）からも隠す。getMutedAuthorKeys が返す `username@domain` キーは、タイムラインの
 * クライアント側除外に加えてリアクション/通知の acct 突き合わせにも使う。
 *
