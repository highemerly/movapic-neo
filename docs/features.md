...
- **投稿完了後のおすすめモーダル**: Web投稿の遷移先（画像詳細 `?posted=1`）で、ブラウザ利用中のユーザーにホーム画面追加を提案する（[InstallSuggestModal](../src/components/pwa/InstallSuggestModal.tsx)）。
  ...
  - 頻度制御は [suggest.ts](../src/lib/pwa/suggest.ts)（localStorage `shamezo.pwa-suggest`）。断られた回数で 30日 → 90日 → 打ち止め。背景タップ/Esc/「いいえ、結構です」/OSダイアログのキャンセルはすべて「断られた」扱い。承諾時は以後 standalone 判定で自然に対象外になるので猶予は記録しない。閉じずに離脱すると猶予が残らないため、同一セッション中の再表示はモジュール変数で抑止する。
