...
# Fediverse サーバーポリシー（すべて任意。詳細は .env.sample 参照）
# LOGIN_PLATFORM="mastodon,misskey"   # ログイン可能なプラットフォーム（未設定=両方）
ALLOWED_SERVERS="handon.club"         # 許可サーバー（カンマ区切り、空の場合は全て許可）
# DENIED_SERVERS=""                   # 拒否サーバー（ログインのみ拒否。既存アカウントは影響なし）
HOME_SERVER="handon.club"             # ホームインスタンス（単一値。所属ユーザーのURLを素の username に）
FAVOR_SERVERS="handon.club"           # 特典サーバー（皆勤賞 grace 4日。未設定=特典なし）
```
