...
## 画像詳細ページのメタ行（MetaItem）
機種名・撮影場所・撮影日時・フォント・投稿ソース・獲得実績など、画像本文の下に並ぶ小さな情報行は**必ず [MetaItem.tsx](../src/app/u/%5Busername%5D/status/%5BimageId%5D/MetaItem.tsx)** を使う。項目ごとに書き下ろすと hover 表現（下線／文字の濃さ）・アイコン間隔・タップ領域がばらつくため。
- 見た目の規約: hover で**下線＋文字を濃く**。遷移とモーダルで表現は変えない。平常時は素のテキストのまま（押せることを示す常時下線は行がうるさくなるので出さない）。
- `as` で描画先を切替: `static`（非対話・span）／`button`（モーダル）／`link`（内部遷移・`linkComponent` で Link 差し替え可）／`external`（別タブ）。`tone="amber"` は実績チップ用（色相は残して「濃くなる」表現だけ揃える）。
- 下線はラベルの span にだけ掛ける。`inline-flex` の直下に `underline` を置くとアイコンの下にも線が伸びる。
- 例外: 撮影場所の削除ボタン（✕）は破壊的操作なので赤へ変化させる従来どおりの扱い。
